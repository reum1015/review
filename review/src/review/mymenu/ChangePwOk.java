...
@WebServlet("/mymenu/change_pw_ok")
public class ChangePwOk extends BaseController {
	...
	public String doRun(HttpServletRequest request, HttpServletResponse response) 
		...
		// 로그인 중이 아니라면 이 페이지를 동작시켜서는 안된다.
			if(web.getSession("loginInfo") == null){
					sqlSession.close();
					web.redirect(web.getRootPath() + "/index", "you should log in.");
					return null;
				}		
				...
				if (!regex.isValue(user_pw)) {
					sqlSession.close();
					web.redirect(null, "Your old password was entered incorrectly. Please enter it again.");
					return null;
				}
				...
				if (regex.isValue(new_user_pw)) {
					if (!regex.isEngNum(new_user_pw) || new_user_pw.length() > 30) {
						sqlSession.close();
					...
					if (!new_user_pw.equals(new_user_pw_re)) {
						sqlSession.close();
						web.redirect(null, "New passwords don’t match");
						return null;
					}
				}
