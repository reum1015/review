...
@WebServlet("/article/article_read_mobile")
public class ArticleReadMobile extends BaseController {
	...
	public String doRun(HttpServletRequest request, HttpServletResponse response) 
		...
		// 조회결과가 존재할 경우 --> 갤러리라면 이미지 경로를 썸네일로 교체(에피소드 리스트)
				if (readArticle != null) {
					for (int i=0; i<readArticle.size(); i++) {
						...
						if (imagePath != null) {
							String thumbPath = upload.createThumbnail(imagePath, 340, 340, true);
		...
		String view = "article/article_read_mobile";		
		return view;
	}
